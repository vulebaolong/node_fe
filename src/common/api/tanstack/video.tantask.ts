...
export const useVideoDetail = (video_id: number) => {
   return useQuery({
      ...
      queryFn: async () => {
         const { data } = await api.get<TRes<TVideo>>(
            `${ENDPOINT.VIDEO.VIDEO_DETAIL()}/${video_id}`
         );
         return data.metaData;
      },
   });
...
export const useVideoComment = () => {
   return useMutation({
      mutationFn: async (payload: TVideoCommentReq) => {
         const { data } = await api.post<TRes<TVideoCommentRes>>(
            `${ENDPOINT.VIDEO.VIDEO_COMMENT()}`,
            payload
         );
         return data.metaData;
      },
...
export const useVideoGetLike = (video_id: number) => {
   return useQuery({
      ...
      queryFn: async () => {
         const { data } = await api.get<TRes<TVideoGetLikeRes>>(
            `${ENDPOINT.VIDEO.VIDEO_GET_LIKE()}/${video_id}`
         );
         return data.metaData;
      },
   });
...
export const useGetTotalLike = (video_id: number) => {
   return useQuery({
      ...
      queryFn: async () => {
         const { data } = await api.get<TRes<number>>(
            `${ENDPOINT.VIDEO.VIDEO_GET_TOAL_LIKE()}/${video_id}`
         );
         return data.metaData;
      },
   });
